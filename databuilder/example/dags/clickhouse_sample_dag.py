...
def connection_string():
    ...
    port = '9000'
    db = 'system'
    return "clickhouse+native://%s:%s@%s:%s/%s" % (user, password, host, port, db)


def create_table_extract_job():
    where_clause_suffix = textwrap.dedent("""
        database not in ('system')
        and table not like '%%_repl'
    """)
    ...
    job_config = ConfigFactory.from_dict({
        f'extractor.clickhouse_metadata.{ClickhouseMetadataExtractor.WHERE_CLAUSE_SUFFIX_KEY}': where_clause_suffix,
        f'extractor.clickhouse_metadata.extractor.sqlalchemy.{SQLAlchemyExtractor.CONN_STRING}': connection_string(),
    ...
    job = DefaultJob(conf=job_config,
                     task=DefaultTask(extractor=ClickhouseMetadataExtractor(), loader=FsNeo4jCSVLoader()),
...
with DAG('amundsen_databuilder', default_args=default_args, **dag_args) as dag:
    clickhouse_table_extract_job = PythonOperator(
        task_id='clickhouse_table_extract_job',
        python_callable=create_table_extract_job
    )

    clickhouse_es_index_job = PythonOperator(
        task_id='clickhouse_es_publisher_sample_job',
        python_callable=create_es_publisher_sample_job
    )

    # elastic search update run after table metadata has been updated
    clickhouse_table_extract_job >> clickhouse_es_index_job
